...
async function renderBus(el, id) {
  ...
  el.innerHTML =
    '<div class="container" style="padding-top:22px;padding-bottom:40px">' +
      ...
      '</div>' +
      '<div class="wa-row">' +
        (mapUrl ? '<a class="map-btn" href="' + mapUrl + '" target="_blank" rel="noopener" title="Open route in Google Maps" aria-label="Open route in Google Maps">' + '<svg viewBox="0 0 24 24" style="width:18px;height:18px;flex:0 0 auto" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z"/><circle cx="12" cy="10" r="3"/></svg>' + '</a>' : '') +
